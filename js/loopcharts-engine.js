...
(function () {
  ...
  var DRAW_TOOLS = [
    ...
    ['text', 'T', 'Text note'],
    ['measure', '\u21F2', 'Measure price / % / bars \u2014 or just hold Shift and drag'],
  ...
  canvasHost.addEventListener('pointerdown', function (event) {
    if (!state.active) return;
    /* hold Shift + click-drag = interval measure, no toolbar needed; the
       measure flow already resets the tool to cursor on pointerup */
    if (event.shiftKey && drawState.tool === 'cursor') { drawState.tool = 'measure'; }

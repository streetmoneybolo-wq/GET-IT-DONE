...
const lab=document.createElement('section');lab.className='academy-visual-math-lab';lab.innerHTML='<div class="academy-math-head"><span>VISUAL MATH LAB · NARRATION SYNCED</span><span id="academy-math-phase">STEP 1 / 3</span></div><div class="academy-math-stage"><div class="academy-math-step"><div class="academy-ladder"><div class="ask"><span>ASK · Seller offers</span><b>$100.05 × 300</b></div><div class="ask"><span>NEXT ASK</span><b>$100.06 × 500</b></div></div><div class="academy-math-arrow">➜</div><div class="academy-ladder"><div class="bid"><span>BID · Buyer offers</span><b>$100.00 × 400</b></div><div class="bid"><span>NEXT BID</span><b>$99.99 × 600</b></div></div><div class="academy-equations"><span>Spread<br><b>$100.05 − $100.00 = $0.05</b></span><span>Midpoint<br><b>($100.05 + $100.00) ÷ 2 = $100.025</b></span><span>Market buy<br><b>Consumes the $100.05 ask; the next available ask can become $100.06</b></span></div></div></div>';
function mountLab(){const deck=document.querySelector('.academy-live-deck');if(deck&&!lab.isConnected)deck.insertAdjacentElement('afterend',lab);return deck}
let guide=null,lastGuide='',guideTarget=null,guideLabel='',guideFrame=0;function positionGuide(){guideFrame=0;if(!guide||!guideTarget||!guideTarget.isConnected)return;const r=guideTarget.getBoundingClientRect(),visible=r.bottom>0&&r.top<innerHeight&&r.right>0&&r.left<innerWidth;guide.style.display=visible?'block':'none';if(!visible)return;guide.style.left=(r.left-4)+'px';guide.style.top=(r.top-4)+'px';guide.style.width=(r.width+8)+'px';guide.style.height=(r.height+8)+'px';guide.dataset.label=guideLabel}function scheduleGuide(){if(!guideFrame)guideFrame=requestAnimationFrame(positionGuide)}function showGuide(target,label){guideTarget=target;guideLabel=label||'';if(!target){guide?.remove();guide=null;return}if(!guide){guide=document.createElement('div');guide.className='academy-guide';document.body.appendChild(guide)}scheduleGuide()}
function syncVisual(detail){mountLab();const text=((detail.lesson?.title||'')+' '+(detail.text||'')+' '+(detail.slide?.visualKind||'')).toLowerCase(),progress=Math.max(0,Math.min(1,Number(detail.progress)||0));lab.style.setProperty('--math-progress',progress);lab.querySelector('#academy-math-phase').textContent='STEP '+Math.min(3,Math.floor(progress*3)+1)+' / 3';const mathNeeded=/bid|ask|spread|order book|price discovery|liquidity|level 2|market order/.test(text);lab.classList.toggle('open',mathNeeded);let key='',target=null,label='';if(/level 2|order book|bid|ask|spread|liquidity/.test(text)){key='depth';target=document.querySelector('.academy-depth')||document.querySelector('.book');label='LEVEL 2: watch price and available size on both sides'}else if(/scanner|s\.i\.r\.e|relative volume|premarket|gainer|momentum/.test(text)){key='scanner';target=document.querySelector('.academy-scanner');label='SCANNER: filter and compare verified live fields'}else if(/candle|ohlc|heikin|renko|kagi|point.{0,3}figure|raindrop|chart pattern/.test(text)){key='chart';target=stage;label='CHART: inspect structure, then validate with context'}if(key!==lastGuide){lastGuide=key;showGuide(target,label)}if(!key)showGuide(null,'')}
function syncFromDeck(){const deck=mountLab();if(!deck)return;const progress=parseFloat(document.querySelector('.academy-deck-progress i')?.style.width||'0')/100;syncVisual({lesson:{title:document.getElementById('lesson-title')?.textContent||''},text:(document.querySelector('.academy-slide-title')?.textContent||'')+' '+(document.querySelector('.academy-caption')?.textContent||''),progress:progress%1,slide:{visualKind:document.querySelector('.academy-slide-visual')?.dataset.kind||''}})}
let watchedDeck=null,deckObserver=null;function attachDeck(){const deck=mountLab();if(!deck||deck===watchedDeck)return;if(deckObserver)deckObserver.disconnect();watchedDeck=deck;deckObserver=new MutationObserver(syncFromDeck);deckObserver.observe(deck,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['style','data-kind']});syncFromDeck()}
window.addEventListener('sml-academy-slide-sync',event=>syncVisual(event.detail||{}));new MutationObserver(attachDeck).observe(document.body,{subtree:true,childList:true});attachDeck();window.addEventListener('sml-academy-market',event=>{latest=Array.isArray(event.detail?.bars)?event.detail.bars:latest;selection=null;stats.classList.remove('open');draw()});new ResizeObserver(()=>{size();scheduleGuide()}).observe(stage);window.addEventListener('scroll',scheduleGuide,{passive:true});window.addEventListener('resize',scheduleGuide,{passive:true});size();
})()</script>`;
}

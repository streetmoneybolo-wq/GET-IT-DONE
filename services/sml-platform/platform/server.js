...
#academy-unlock{position:fixed;z-index:2147483647;right:18px;bottom:18px;margin:0;padding:12px 16px;border:2px solid #75f5bf;border-radius:10px;background:#00c47d;color:#042217;font-size:.85rem;box-shadow:0 10px 30px rgba(0,0,0,.55),0 0 20px rgba(0,196,125,.45)}
</style>
...
</script></body></html>`.replace('setInterval(()=>location.reload(),30000)', "let refreshPending=false;const keepWarm=async()=>{if(document.hidden||refreshPending)return;refreshPending=true;try{const query=new URLSearchParams(location.search),symbol=query.get('symbol')||'SPY',tf=query.get('tf')||'5m';const response=await fetch('/academy-activity/market?symbol='+encodeURIComponent(symbol)+'&tf='+encodeURIComponent(tf),{cache:'no-store'});document.getElementById('status').textContent=response.ok?'LIVE':'RETRY'}catch{document.getElementById('status').textContent='RETRY'}finally{refreshPending=false}};setInterval(keepWarm,15000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)keepWarm()})").replace('</body></html>', `<script>(()=>{const unlock=document.getElementById('academy-unlock'),state=document.querySelector('.market-state');if(!unlock||!state)return;let open=false;const render=()=>{document.body.classList.toggle('academy-tools-open',open);unlock.textContent=open?'Close Academy Tools':'Unlock Academy Tools';unlock.setAttribute('aria-pressed',String(open));state.textContent=open?'LIVE INTERACTIVE ACADEMY':'READ-ONLY TRAINING';state.style.color=open?'#52e6ad':'#ffbf5d'};unlock.onclick=()=>{open=!open;render()};render()})()</script></body></html>`).replace('</body></html>', `<script>(()=>{const shell=document.querySelector('.shell');if(!shell)return;const query=new URLSearchParams(location.search),symbol=(query.get('symbol')||'SPY').toUpperCase().replace(/[^A-Z0-9.:-]/g,'').slice(0,10)||'SPY';const frame=document.createElement('iframe');frame.className='academy-dashboard-frame';frame.title='StockMarketLoop Academy Chart Lab';frame.src='https://stockmarketloop.com/analyst-dashboard/?academy=1&symbol='+encodeURIComponent(symbol);frame.allow='fullscreen';frame.referrerPolicy='origin';shell.replaceChildren(frame);const style=document.createElement('style');style.textContent='.shell{display:block!important;min-height:0!important;padding:0!important}.academy-dashboard-frame{display:block;width:100%;height:100%;min-height:720px;border:0;background:#06090d}.chart,.side{display:none!important}body.academy-tools-open{overflow:hidden}body.academy-tools-open .academy-dashboard-frame{position:fixed;inset:0;z-index:2147483000;width:100vw;height:100vh;min-height:100vh;background:#06090d}body.academy-tools-open #academy-unlock{z-index:2147483647;top:14px;right:14px;bottom:auto}@media(max-width:720px){.academy-dashboard-frame{min-height:640px}}';document.head.appendChild(style)})()</script></body></html>`);
}

...
async function getAcademyCandles(symbol, timeframe = '5m') {
  ...
  academyMarketCache.set(cacheKey, { expiresAt: Date.now() + 15_000, payload });
  return payload;
}
...
function createServer({ checkDatabase, acceptWordPressEvent, wordpressWebhookSecret = '',
  ...
  return http.createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && path === '/academy-activity/') {
      ...
      try {
        sendHtml(response, 200, academyActivityHtml(await getAcademyCandles(symbol, timeframe)));
      } catch (error) {
        logger('warn', 'academy_activity_initial_market_failed', { error });
        sendHtml(response, 200, academyActivityHtml({ symbol, tf: timeframe, bars: [] }));
      }
      return;
    }

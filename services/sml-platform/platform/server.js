...
function createServer({ checkDatabase, acceptWordPressEvent, wordpressWebhookSecret = '',
  ...
  return http.createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && path === '/academy-activity/') {
      const symbol = new URL(request.url || '/', 'http://localhost').searchParams.get('symbol') || 'SPY';
      try {
        sendHtml(response, 200, academyActivityHtml(await getAcademyCandles(symbol)));
      } catch (error) {
        logger('warn', 'academy_activity_initial_market_failed', { error });
        sendHtml(response, 200, academyActivityHtml({ symbol, bars: [] }));
      }
      return;
    }

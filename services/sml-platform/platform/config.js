...
function getConfig(env = process.env) {
  ...
  return Object.freeze({
    ...
    // --- private member email analytics (fail-closed until all required secrets exist) ---
    memberEmailEnabled: String(env.SML_MEMBER_EMAIL_ENABLED || '').trim() === '1',
    memberEmailEncryptionKey: String(env.SML_EMAIL_ENCRYPTION_KEY || '').trim(),
    memberEmailHashKey: String(env.SML_EMAIL_HASH_KEY || '').trim(),
    resendApiKey: String(env.RESEND_API_KEY || '').trim(),
    memberEmailFrom: String(env.SML_EMAIL_FROM || '').trim(),
    memberEmailReplyTo: String(env.SML_EMAIL_REPLY_TO || '').trim(),
    memberEmailBusinessAddress: String(env.SML_EMAIL_BUSINESS_ADDRESS || '').trim(),
    ...
    discordDisputeAppId: String(env.SML_DISPUTE_BOT_APP_ID || '').trim(),
    discordDisputeBotToken: String(env.SML_DISPUTE_BOT_TOKEN || '').trim(),

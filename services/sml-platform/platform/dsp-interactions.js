...
function createDailySocialPayoutsInteractions({ config, pool } = {}) {
  ...
  async function handle(request, response, rawBody) {
    ...
    if (!Buffer.isBuffer(rawBody) || rawBody.length > MAX_BODY_BYTES) { send(response, 413, { error: 'payload_too_large' }); return; }
    const signature = String(request.headers['x-signature-ed25519'] || '');
    const timestamp = String(request.headers['x-signature-timestamp'] || '');
    let valid = false;
    try { valid = SIGNATURE.test(signature) && timestamp.length > 0 && crypto.verify(null, Buffer.concat([Buffer.from(timestamp), rawBody]), publicKey, Buffer.from(signature, 'hex')); } catch (_) { valid = false; }
    if (!valid) { send(response, 401, { error: 'invalid_signature' }); return; }
    let interaction;
    try { interaction = JSON.parse(rawBody.toString('utf8')); } catch (_) { send(response, 400, { error: 'invalid_json' }); return; }
